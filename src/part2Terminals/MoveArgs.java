...
public class MoveArgs  implements RobotProgramNode {
	...
	public MoveArgs (Scanner s) {
		try {
			if (s.hasNext(Pattern.compile("-?\\d+"))) {
				this.iterations = Integer.parseInt(s.next());
			} else {
				if (!parseSenOrOp(s)) {
					throw new ParserFailureException("Failed at parsing optional argument sensor/operation for move");
				}
			}
		} catch (NoSuchElementException e) {
			throw new ParserFailureException("Failed at parsing optional argument for move");
		}
	}
	...
	public String toString(int depth) {
		...
		if (sensor.equals("") && operation == null) {
			builder.append("move("+iterations+")\n");
		} else {
			if (isSen) {
				builder.append("move("+sensor+")\n");
			} else {
				builder.append("move("+operation.toString()+")\n");
			}
		}
